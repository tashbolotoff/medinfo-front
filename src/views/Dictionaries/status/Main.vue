<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full">
      <h1 class="text-xl font-bold">Справочник "Статусы"</h1>
    </div>
    <!--table-->
...
async function getByPagination(page) {
  try {
    await (async () => {
      DictionariesService.getStatusesWithPagination({page: page, size: 10}).then(response => {
...
function confirmDelete() {
  DictionariesService.deleteStatus({id: objectForDelete.value.id}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  DictionariesService.editStatus(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  DictionariesService.createStatus(modalParams.datas).then(response => {
    loading.value = false
    createToast({
