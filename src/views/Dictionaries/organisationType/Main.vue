<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full">
      <h1 class="text-xl font-bold">Справочник "Категория медицинских изделий"</h1>
    </div>
    <!--table-->
      ...
      <div class="overflow-auto lg:overflow-visible mt-8 sm:mt-0">
        <table class="table sm:mt-2 bg-white rounded-xl">
          ...
          <tr class="bg-gray-200 text-gray-700">
            <th>Вид контрацепции</th>
            <th>Вид мед.изделия</th>
            <th>Название</th>
      ...
      <div class="modal-content">
        ...
        <form v-on:submit.prevent="modalParams.isEditModal ? edit() : create()">
          <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
            ...
            <div class="col-span-12">
              <label class="form-label">Вид контрацепции</label>
              <Select2 v-model="modalParams.datas.contraceptionTypeId" :options="dictionaries.contraceptionType"
                       :settings="select2Settings"/>
            </div>
            <div class="col-span-12">
              <label class="form-label">Вид мед.изделий</label>
              <Select2 v-model="modalParams.datas.dictMiTypeId" :options="dictionaries.dictMiType"
                       :settings="select2Settings"/>
            </div>
            <!-- Код-->
          </div>
...
async function getByPagination(page) {
  try {
    await (async () => {
      await $api.get('/Sklad/ListCategoryMi',
        {
          params: {
            page: page,
            size: 10
...
function showEditModal(item) {
  ...
  modalParams.datas.contraceptionTypeId = item.contraceptionTypeId
  modalParams.datas.dictMiTypeId = item.dictMiTypeId
}
...
function confirmDelete() {
  $api.delete('/Sklad/DeleteCategoryMi', {
    params: {
      id: objectForDelete.value.id
    }
  }).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  $api.put('/Sklad/EditCategoryMi/', {
    id: modalParams.datas.id,
    name: modalParams.datas.name,
    contraceptionTypeId: modalParams.datas.contraceptionTypeId,
    dictMiTypeId: modalParams.datas.dictMiTypeId
  }).then(response => {
    loading.value = false
    createToast({
...
function resetData() {
  ...
  modalParams.datas.contraceptionTypeId = null
  modalParams.datas.dictMiTypeId = null
}
