<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full">
      <h1 class="text-xl font-bold">Справочник "Сообщения"</h1>
    </div>
    <!--table-->
      ...
      <div class="overflow-auto lg:overflow-visible mt-8 sm:mt-0">
        <table class="table sm:mt-2 bg-white rounded-xl">
          ...
          >
            ...
            <td class="border-b dark:border-dark-5">
              <template v-if="item.timeType === 0">День</template>
              <template v-if="item.timeType === 1">Месяц</template>
              <template v-if="item.timeType === 2">Год</template>
            </td>
            <td class="border-b dark:border-dark-5">
              {{ item.text }}
            </td>
        ...
        <form v-on:submit.prevent="modalParams.isEditModal ? edit() : create()">
          <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
            ...
            <div class="col-span-12">
              ...
              <Select2 v-model="modalParams.datas.categoryId" :options="dictionaries.categories" :settings="select2Settings" />
            </div>
            <!-- Название-->
            <div class="col-span-12">
              <label class="form-label">Период отправки <span style="color: red">*</span></label>
              <Select2 v-model="modalParams.datas.timeType" :options="[{id: 0, text: 'День'},{id: 1, text: 'Месяц'},{id: 2, text: 'Год'},]" :settings="select2Settings" />
            </div>
            <!-- Название-->
            <div class="col-span-12">
...
async function getByPagination(page) {
  try {
    await (async () => {
      MessageService.getMessagesByPage(page, genResult.pageSize).then(response => {
