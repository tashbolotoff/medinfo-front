<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full">
      <h1 class="text-xl font-bold">Справочник "Отрасли"</h1>
    </div>
    <!--table-->
...
async function getByPagination(page) {
  try {
    await (async () => {
      DictionariesService.getOtraslsWithPagination({page: page, size: 10}).then(response => {
...
function confirmDelete() {
  DictionariesService.deleteOtrasl({id: objectForDelete.value.id}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  DictionariesService.editOtrasl(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  DictionariesService.createOtrasl(modalParams.datas).then(response => {
    loading.value = false
    createToast({
