<template>
  <div class="grid grid-cols-12 gap-4 mt-5">
    <div class="col-span-full">
      <h1 class="text-xl font-bold">Категории и подкатегории</h1>
    </div>
    <!--table-->
      ...
      <div class="overflow-auto lg:overflow-visible mt-8 sm:mt-0">
        <table class="table sm:mt-2 bg-white rounded-xl">
          ...
          <tr class="bg-gray-200 text-gray-700">
            ...
            <th class="text-center whitespace-nowrap">Подкатегории</th>
          </tr>
          </thead>
          <tbody>
          <tr
          ...
          >
            ...
            <td class="table-report__action border-b dark:border-dark-5">
              <div class="flex justify-center items-center">
                ...
                <a class="flex items-center mr-3"
                  ...
                  <PlusSquareIcon class="w-4 h-4 mr-1"/>
                  Подкатегория
                </a>
                <a
            ...
            <td class="table-report__action border-b dark:border-dark-5">
              <table class="table sm:mt-2 bg-white rounded-xl" v-if="item.childs.length != 0">
                <thead>
                <tr class="bg-gray-200 text-gray-700">
                  <th>Название</th>
                  <th class="text-center whitespace-nowrap">Действия</th>
...
async function getByPagination(page) {
  try {
    await (async () => {
      CategoriesService.getCategoriesWithPagination({page: page, size: 10}).then(response => {
...
function showCreateModal(category) {
  ...
  modalParams.datas.parentId = category ? category.id : null
}
...
function showEditModal(item) {
  ...
  modalParams.datas.parentId = item.parentId
}

function showModalDelete(item) {
  objectForDelete.value = item
  console.log(objectForDelete.value)
}

function confirmDelete() {
  CategoriesService.deleteCategories({id: objectForDelete.value.id}).then(response => {
    createToast({
...
function edit() {
  loading.value = true
  CategoriesService.editCategories(modalParams.datas).then(response => {
    loading.value = false
    createToast({
...
function create() {
  loading.value = true
  CategoriesService.createCategories(modalParams.datas).then(response => {
    loading.value = false
    createToast({
